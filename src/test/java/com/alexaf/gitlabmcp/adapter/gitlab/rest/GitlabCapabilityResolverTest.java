...
class GitlabCapabilityResolverTest {
    ...
    void enablesMergeRequestDiffsStartingWithGitlab15_7() {
        ...
        assertThat(resolver.resolve("15.7.0-ee", null).capabilities()).contains(GitlabCapability.MERGE_REQUEST_DIFFS);
    }

    @Test
    void enablesArtifactTreeStartingWithGitlab18_8() {
        assertThat(resolver.resolve("18.7.9-ee", null).capabilities()).doesNotContain(GitlabCapability.ARTIFACT_TREE);
        assertThat(resolver.resolve("18.8.0-ee", null).capabilities()).contains(GitlabCapability.ARTIFACT_TREE);
    }
