...
class MavenTraceFailureAnalyzerTest {
    ...
    private final MavenTraceFailureAnalyzer analyzer =
            new MavenTraceFailureAnalyzer(new MavenFailureAnalyzer());
    ...
    private PipelineContext context(Map<Long, String> traces) {
        ...
        Job job = new Job(7L, "test", "test", "failed", "script_failure", null,
                "main", false, false, null, null, null, 1.0, 1.0, List.of());
        return new PipelineContext(pipeline, List.of(job), false, 1)
                .withExecutionData(traces, Map.of());
    }
}
