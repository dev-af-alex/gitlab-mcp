...
class GitlabResourcesTest {
    ...
    void jobTraceDelegatesToGateway() {
        when(gitlab.getJobTraceTail("group/repo", "job-url", 60_000)).thenReturn("trace");
    ...
    void jobArtifactFileDecodesArtifactPathAndDelegatesToGateway() {
        when(gitlab.getJobArtifactFile("group/repo", "job-url", "target/surefire-reports/TEST.xml", 60_000))
                .thenReturn("artifact");
        ...
        verify(gitlab).getJobArtifactFile("group/repo", "job-url", "target/surefire-reports/TEST.xml", 60_000);
    }
}
