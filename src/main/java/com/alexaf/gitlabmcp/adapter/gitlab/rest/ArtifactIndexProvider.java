...
public class ArtifactIndexProvider {
    ...
    public ArtifactIndexProvider(GitlabApiClient gitlab, GitlabServerInfoProvider serverInfoProvider) {
        this.gitlab = gitlab;
        this.serverInfoProvider = serverInfoProvider;
    }

    public List<ArtifactFile> list(String jobApi, String path, Boolean recursive, GitlabPageRequest page) {
    ...
    public List<ArtifactFile> find(String jobApi, String pattern, Boolean regex, GitlabPageRequest page) {
        if (!supportsArtifactTree()) {
            return legacyFind(jobApi, pattern, regex, page);
        }
        try {
            Pattern compiled = compilePathPattern(pattern, regex);
            List<ArtifactFile> matches = gitlab
                    .getAllPages(
                            jobApi + "/artifacts/tree",
                            ArtifactFile.class,
    ...
    private List<ArtifactFile> legacyList(String jobApi, String path, Boolean recursive, GitlabPageRequest page) {
        return gitlab.listArtifactArchive(jobApi + "/artifacts", path, recursive, pageValue(page), perPageValue(page));
    }

    private List<ArtifactFile> legacyFind(String jobApi, String pattern, Boolean regex, GitlabPageRequest page) {
        return gitlab.findArtifactArchiveFiles(
                jobApi + "/artifacts", pattern, regex, pageValue(page), perPageValue(page));
    }

    private Pattern compilePathPattern(String pattern, Boolean regex) { // NOPMD - existing complexity baseline
        ...
        for (int i = 0; i < effectivePattern.length(); i++) {
            ...
            if (character == '*') {
                boolean doubleStar = i + 1 < effectivePattern.length() && effectivePattern.charAt(i + 1) == '*';
