...
public class GitlabMcpConfiguration {
    ...
    @Bean
    RestClient.Builder restClientBuilder(GitlabProperties properties, ObjectProvider<SslBundles> sslBundlesProvider) {
        ...
        if (StringUtils.hasText(properties.proxyUrl())) {
            ...
            int port = proxy.getPort() >= 0 ? proxy.getPort() : defaultPort(proxy.getScheme());
        ...
        if (StringUtils.hasText(properties.sslBundle())) {
            ...
            if (sslBundles == null) {
                throw new IllegalArgumentException("GITLAB_SSL_BUNDLE is set but no Spring SSL bundles are configured");
            }
            httpClient.sslContext(
                    sslBundles.getBundle(properties.sslBundle().trim()).createSslContext());
        }
