...
public class GitlabJobTools {
    ...
    public String listJobArtifacts(
            ...
            @McpToolParam(description = "Job id or GitLab job URL.")
            String jobId,
            @McpToolParam(description = "Path inside the artifacts archive to browse.", required = false)
            String path,
            @McpToolParam(description = "Whether to list entries recursively.", required = false)
            Boolean recursive,
    ...
    public String findJobArtifactFiles(
            ...
            @McpToolParam(description = "Job id or GitLab job URL.")
            String jobId,
            @McpToolParam(description = "Glob or regex path pattern. Example: **/TEST-*.xml.")
            String pattern,
            @McpToolParam(description = "Treat pattern as Java regex instead of glob. Defaults to false.", required = false)
            Boolean regex,
            @McpToolParam(description = "Page number, starting from 1.", required = false)
            Integer page,
            @McpToolParam(description = "Results per page. Capped by GITLAB_MAX_PER_PAGE.", required = false)
    ...
    @McpTool(
            ...
            description = "Get one text file from a GitLab job artifacts archive, redacted and optionally truncated. Read-only.",
            annotations = @McpAnnotations(readOnlyHint = true, destructiveHint = false, idempotentHint = true))
    public String getJobArtifactFile(
            ...
            @McpToolParam(description = "Job id or GitLab job URL.")
            String jobId,
            @McpToolParam(description = "Path to a text file inside the artifacts archive.")
            String artifactPath,
