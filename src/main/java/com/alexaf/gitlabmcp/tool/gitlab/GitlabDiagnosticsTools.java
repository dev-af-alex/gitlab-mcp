...
public class GitlabDiagnosticsTools {
    ...
    @McpTool(
            ...
            description = "Search the redacted tail of a GitLab job trace and return matching lines with context. Read-only.",
    ...
    public String getJobTraceMatches(
            ...
            String projectId,
            @McpToolParam(description = "Job id or GitLab job URL.")
            String jobId,
            @McpToolParam(description = "Text or regex pattern to search in the trace tail.")
            String pattern,
    ...
    @McpTool(
            ...
            description = "Extract a compact Maven/Surefire-aware failure summary from a failed GitLab job trace and reports; full evidence is opt-in. Read-only.",
            annotations = @McpAnnotations(readOnlyHint = true, destructiveHint = false, idempotentHint = true))
    ...
    @McpTool(
            ...
            description = "Analyze failed Maven test classes selected from a job log and read bounded Surefire .txt or TEST-*.xml reports. Read-only.",
            annotations = @McpAnnotations(readOnlyHint = true, destructiveHint = false, idempotentHint = true))
    ...
    @McpTool(
            ...
            description = "Analyze a failed GitLab pipeline with compact root cause, failed tests, and cascade grouping; full context and diffs are opt-in. Read-only.",
            annotations = @McpAnnotations(readOnlyHint = true, destructiveHint = false, idempotentHint = true))
    public String analyzeFailedPipeline(
            @McpToolParam(description = "Project id or full path.")
            String projectId,
            @McpToolParam(description = "Pipeline id or GitLab pipeline URL. Optional when mergeRequestIid is set.", required = false)
            String pipelineId,
            @McpToolParam(description = "Merge request IID, !IID, or GitLab merge request URL. Used when pipelineId is not set.", required = false)
            ...
            @McpToolParam(description = "Whether to include raw trace text in the response. Defaults to false.", required = false)
            Boolean includeRawTraces,
            @McpToolParam(description = "Whether to include likely useful artifact hints. Defaults to true.", required = false)
    ...
    @McpTool(
            ...
            description = "Analyze the latest failed merge request pipeline with compact root cause, failed tests, cascade grouping, and changed-file correlation; full context and diffs are opt-in. Read-only.",
            annotations = @McpAnnotations(readOnlyHint = true, destructiveHint = false, idempotentHint = true))
    public String analyzeMergeRequestPipelineFailure(
            @McpToolParam(description = "Project id or full path.")
            String projectId,
            @McpToolParam(description = "Merge request IID, !IID, or GitLab merge request URL.")
            String mergeRequestIid,
            @McpToolParam(description = "Maximum trace bytes per failed job to inspect. Defaults to 60000.", required = false)
            Integer maxTraceBytesPerJob,
            @McpToolParam(description = "Whether to include raw trace text in the response. Defaults to false.", required = false)
            Boolean includeRawTraces,
            @McpToolParam(description = "Include full trace contexts, report evidence, and merge-request diffs. Defaults to false.", required = false)
