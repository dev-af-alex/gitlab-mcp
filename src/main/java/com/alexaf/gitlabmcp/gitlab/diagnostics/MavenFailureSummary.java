...
public record MavenFailureSummary(
    ...
    public boolean executionFailureDetected() {
        return evidence.stream().anyMatch(line -> line.contains("forked VM terminated")
                || line.contains("Process Exit Code:")
                || line.contains("SurefireBooterForkException"));
    }

    public boolean compilationFailureDetected() {
        return evidence.stream().anyMatch(line -> line.contains("COMPILATION ERROR")
    ...
    public MavenFailureSummary compact() {
        return new MavenFailureSummary(
                ...
                evidence.stream().map(MavenFailureSummary::compactLine).distinct().limit(8).toList(),
                surefireHints.stream().map(MavenFailureSummary::compactLine).distinct().limit(4).toList(),
