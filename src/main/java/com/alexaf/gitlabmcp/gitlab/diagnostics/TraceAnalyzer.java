...
public class TraceAnalyzer {
    ...
    private final List<Rule> rules = List.of(
            new Rule(
                    "Maven test compilation failure",
                    "high",
                    line -> containsAny(line, "compilation error", "maven-compiler-plugin", "compilation failure"),
                    List.of(
                            "Inspect the compiler error and restore the missing or incompatible test source dependency.")),
            new Rule(
                    "Maven/JUnit test failure",
                    "high",
                    line -> containsAny(
                            line, "tests run:", "there are test failures", "surefire-reports", "build failure"),
            ...
            new Rule(
                    ...
                    List.of(
                            "Inspect the stack trace and check the application code around the first caused-by section.")),
            new Rule(
                    "Node.js/npm failure",
                    "medium",
            ...
            new Rule(
                    ...
                    "high",
                    line -> containsAny(
                            line, "401 unauthorized", "403 forbidden", "permission denied", "access denied"),
        ...
        for (Rule rule : rules) {
            if (matchesAnyLine(lines, rule)) {
                ...
                return new TraceAnalysis(
                        rule.detectedCause(), rule.confidence(), List.copyOf(evidence), rule.nextSteps());
            }
        }
    ...
    private record Rule(String detectedCause, String confidence, Predicate<String> matches, List<String> nextSteps) {}
}
